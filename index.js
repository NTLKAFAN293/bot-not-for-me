        ...
        if (interaction.customId.startsWith('open_ticket_')) {
            ...
            try {
                ...
                const supportButton = new ButtonBuilder()
                    .setCustomId(`support_ticket_${ticketChannel.id}`)
                    .setLabel('طــلــب دعــم')
                    .setStyle(ButtonStyle.Primary);

                const callButton = new ButtonBuilder()
                    .setCustomId(`call_support_${ticketChannel.id}`)
...
client.on('messageCreate', async (message) => {
    ...
    const hasTimeoutCmd = timeoutCommands.some(c => c.command.toLowerCase() === command);
    
    if (hasTimeoutCmd || command === 'تايم-اوت' || command === 'timeout') {
    ...
    const hasWarningCmd = warningCommands.some(c => c.command.toLowerCase() === command);
    
    if (hasWarningCmd || command === 'تحذير' || command === 'warning') {
        if (!message.member.permissions.has(PermissionFlagsBits.ModerateMembers)) {
            return message.reply('مــا عــنــدك صــلاحــيــات');
        }
        ...
        const reason = args.slice(2).join(' ') || 'لا يــوجــد ســبــب';
