...
const commands = [
    ...
    new SlashCommandBuilder()
        .setName('untimeout')
        .setDescription('فــك الــتــايــم اوت عــن عــضــو')
        .addUserOption(option =>
            option.setName('العضو')
                .setDescription('الــعــضــو الــي بــتــفــك عــنــه الــتــايــم')
                .setRequired(true))
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'untimeout') {
            ...
            if (!member) {
                return interaction.reply({ content: 'مــا لــقــيــت الــعــضــو', ephemeral: true });
            }

            try {
                await member.timeout(null);
                await interaction.reply({ content: `تــم فــك الــتــايــم عــن <@${member.id}> <:1000060658:1423580120551264347>`, ephemeral: true });
            } catch (error) {
                console.error(error);
                await interaction.reply({ content: 'صــار خــطــأ', ephemeral: true });
            }
